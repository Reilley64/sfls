    ...
    if media.thumbnail_file.is_none() {
        return Err(Problem {
            r#type: "https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/500"
                .to_string(),
            title: "Thumbnail not found".to_string(),
            status: 404,
            detail: Some(format!("Thumbnail for Media with id {media_id} not found")),
            instance,
        });
    }

    let mut components = vec![media.thumbnail_file, media.path];
